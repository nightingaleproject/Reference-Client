...
namespace NVSSClient.tests 
{
    ...
    public class VRDRRecordControllerShould : IClassFixture<CustomWebApplicationFactory<NVSSClient.Startup>>
    {
        private readonly CustomWebApplicationFactory<NVSSClient.Startup> _factory;
        private readonly HttpClient _client;
    
        public VRDRRecordControllerShould(CustomWebApplicationFactory<NVSSClient.Startup> factory)
        {
            _factory = factory;
        ...
        public async Task PostRecords_ShouldSaveMessage()
        {
            ...
            HttpRequestMessage postRequest = new HttpRequestMessage(HttpMethod.Post, "http://localhost:4300/vrdrrecord/submissions");
        ...
        public async Task PostRecord_ShouldSaveMessage()
        {
            var records = GetTestRecords("test-files/json/DeathRecord1.json");
            // Submit that Death Record
            HttpRequestMessage postRequest = new HttpRequestMessage(HttpMethod.Post, "http://localhost:4300/vrdrrecord/submission");
        ...
        public async Task PostAliasRecords_ShouldSaveMessage()
        {
            ...
            // Submit that Death Record
            HttpRequestMessage postRequest = new HttpRequestMessage(HttpMethod.Post, "http://localhost:4300/vrdrrecord/aliases");
        ...
        public async Task PostAliasRecord_ShouldSaveMessage()
        {
            ...
            // Submit that Death Record
            HttpRequestMessage postRequest = new HttpRequestMessage(HttpMethod.Post, "http://localhost:4300/vrdrrecord/alias");
        ...
        public async Task PostVoidRecords_ShouldSaveMessage()
        {
            ...
            // Submit that Death Record
            HttpRequestMessage postRequest = new HttpRequestMessage(HttpMethod.Post, "http://localhost:4300/vrdrrecord/voids");
            var json = JsonSerializer.Serialize(dataList);
            postRequest.Content = new StringContent(json);
            postRequest.Content.Headers.ContentType = new MediaTypeHeaderValue("application/json");
            var response = await _client.SendAsync(postRequest);
        ...
        public async Task PostVoidRecord_ShouldSaveMessage()
        {
            ...
            // Submit that Death Record
            HttpRequestMessage postRequest = new HttpRequestMessage(HttpMethod.Post, "http://localhost:4300/vrdrrecord/void");
            var json = JsonSerializer.Serialize(data);
            postRequest.Content = new StringContent(json);
            postRequest.Content.Headers.ContentType = new MediaTypeHeaderValue("application/json");
            var response = await _client.SendAsync(postRequest);
        ...
        {
            var response = await _client.GetAsync("/vrdrrecord/2020/NY/182");
        ...
        {
            var response = await _client.GetAsync("/vrdrrecord");
            response.EnsureSuccessStatusCode();
            var responseString = await response.Content.ReadAsStringAsync();
            //Assert, for now just check it's not empty
            Assert.False(String.IsNullOrWhiteSpace(responseString));
