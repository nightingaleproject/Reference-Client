...
namespace NVSSClient.Controllers
{
    ...
    public class FTDRRecordController : ControllerBase
    {
        ...
        private readonly string ftdrIJEVersion = Startup.StaticConfig.GetValue<string>("SupportedIGVersions:FetalDeathRecord");
        ...
        {               
            try {
                foreach (object text in textList)
                {
                    ...
                    FetalDeathRecord record = new FetalDeathRecord(text.ToString(), true);
                    var message = new FetalDeathRecordSubmissionMessage(record);
                    InsertFDRMessageItem(message);
                }
            } catch (Exception e){
        ...
        {               
            try {
                // Create a submission message for the record
                FetalDeathRecord record = new FetalDeathRecord(recordJson.ToString(), true);
                var message = new FetalDeathRecordSubmissionMessage(record);
                InsertFDRMessageItem(message);
        ...
        {             
            try {
                foreach (object text in textList)
                {
                    // Create a submission message for the record
                    FetalDeathRecord record = new FetalDeathRecord(text.ToString(), true);
                    var message = new FetalDeathRecordUpdateMessage(record);
                    InsertFDRMessageItem(message);
                }
            } catch (Exception e){
                Console.WriteLine("Error Handling Record: {0}", e);
        ...
        {               
            try {
                foreach (object text in textList)
                {
                    ...
                    var message = new FetalDeathRecordVoidMessage(record);
        ...
        // InsertFDRMessageItem inserts the given message into the MessageItem table to get picked up by the TimedHostedService
        public void InsertFDRMessageItem(BFDRBaseMessage message){
            try
            {
                using (var scope = _scopeFactory.CreateScope()){
                    var context = scope.ServiceProvider.GetRequiredService<AppDbContext>();
                    ...
                    item.IJE_Version = ftdrIJEVersion;
